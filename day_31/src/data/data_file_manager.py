...
class DataFileManager:
    ...
    def read_file(self) -> pd.DataFrame:
        ...
        try:
            print(f"Attempting to read file: {self._file_name}")
            data = pd.read_json(self._file_name, orient="records")
            if data.empty:
                print(f"Warning: {self._file_name} is empty.")
            return data
        except ValueError as e:  # If the JSON file is empty or invalid
            print(f"ValueError: Invalid or empty JSON file at {self._file_name}. Details: {e}")
            return pd.DataFrame([], columns=["words", "is_checked"])
        except FileNotFoundError as e:
            print(f"FileNotFoundError: {self._file_name} not found. Details: {e}")
            raise
        except Exception as e:
            print(f"Unexpected error reading {self._file_name}: {e}")
            raise
